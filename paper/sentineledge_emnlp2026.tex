...
$\HeadlineDim$-feature, $\HeadlineSize$~MB hand-crafted feature
classifier head reaches per-call streaming $F_1 = \HeadlineFOne$ at
$\HeadlineLat$~ms p50 per-sentence latency on a single CPU thread
($\HeadlineThroughput$~sentences/sec). Per-sentence probabilities
are smoothed by an exponential moving average; alerts fire when the
smoothed score crosses a threshold, flagging the median scam within
$\TtdMedianSent$ sentences ($\approx\TtdMedianSec$~s of speech). We
benchmark this minimal classifier against TF--IDF, combined-feature,
SVM, XGBoost, and DistilBERT~\citep{Sanh2020DistilBERT} variants
inside the same streaming framework, and find that linear models on
hand-crafted features match or exceed heavier classifiers on
clean data. We further measure robustness to ASR-style perturbations
and adversarial scam phrasings, report adversarial retraining for
both the linear and tree-based heads, and quantify zero-shot
...
(bolded) matches or exceeds every heavier classifier on clean
streaming $F_1$, while requiring orders of magnitude less compute
and storage. DistilBERT achieves competitive streaming quality but
at $\SpeedupVsDistil\times$ higher latency and
$\SizeRatioVsDistil\times$ larger model size. XGBoost on the union
of features ($518$-dim) reaches $F_1 = \StreamFOne$ on clean data;
